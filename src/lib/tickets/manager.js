...
module.exports = class TicketManager {
	...
	async create({
      ...
      interaction.member ?? (await guild.members.fetch(interaction.user.id));
		...
		if (category.guild.blocklist.length !== 0) {
			const blocked = category.guild.blocklist.some(r =>
		...
		if (category.requiredRoles.length !== 0) {
			const missing = category.requiredRoles.some(
			...
			return await interaction.reply({
				embeds: [
					new ExtendedEmbedBuilder({
						...
						.setColor(category.guild.errorColour)
						.setTitle(
							getMessage('misc.member_limit.title', memberCount, memberCount),
						)
						.setDescription(
							getMessage('misc.member_limit.description', memberCount),
						),
				],
				ephemeral: true,
			});
			...
			return await interaction.reply({
				embeds: [
					new ExtendedEmbedBuilder({
						...
						.setDescription(
							getMessage('misc.cooldown.description', { time: ms(cooldown - Date.now()) }),
						),
				],
				ephemeral: true,
			});
											...
											.setOptions(
												q.options.map((o, i) => {
													const builder = new StringSelectMenuOptionBuilder()
														...
														.setLabel(o.label);
													if (o.description) {
														builder.setDescription(o.description);
													}
													if (o.emoji) {
														builder.setEmoji(
															emoji.hasEmoji(o.emoji)
																? emoji.get(o.emoji)
																: { id: o.emoji },
														);
													}
   ...
   */
	async postQuestions({
				...
				answers = category.questions
					...
					.map(q => ({
						...
						value: interaction.fields.getTextInputValue(q.id)
							? encrypt(interaction.fields.getTextInputValue(q.id))
							: '',
					}));
				if (category.customTopic) {
					topic = interaction.fields.getTextInputValue(category.customTopic);
				}
		...
		const number =
      (await this.client.prisma.ticket.count({ where: { guildId: category.guild.id } })) + 1;
		...
		if (!stats) {
			...
			stats = {
				avgResolutionTime: ms(
					tickets.reduce(
						(total, ticket) => total + (ticket.closedAt - ticket.createdAt),
						0,
					) ?? 1 / tickets.length,
					{ long: true },
				),
				avgResponseTime: ms(
					tickets.reduce(
						(total, ticket) =>
							total + (ticket.firstResponseAt - ticket.createdAt),
						0,
					) ?? 1 / tickets.length,
					{ long: true },
				),
				...
				new ExtendedEmbedBuilder()
					...
					.setFields(
						category.questions.map(q => ({
							name: q.label,
							value:
                interaction.fields.getTextInputValue(q.id) ||
                getMessage('ticket.answers.no_value'),
						})),
					),
			);
		} else if (topic) {
		...
		if (pinned.system) {
			pinned
				...
				.catch(() =>
					this.client.log.warn('Failed to delete system pin message'),
				);
		}
		...
		if (referencesMessage) {
			...
			message = await (
				await this.client.channels.fetch(referencesMessage[0])
			)?.messages.fetch(referencesMessage[1]);
			if (message) {
				...
				if (message.system) {
					...
					if (!message.member) {
						try {
							...
							this.client.log.verbose(
								'Failed to fetch member %s of %s',
								message.author.id,
								message.guild.id,
							);
						}
		...
		await interaction.editReply({
			...
			embeds: [
				new ExtendedEmbedBuilder({
					...
					.setDescription(
						getMessage('ticket.created.description', { channel: channel.toString() }),
					),
			],
		});
		...
		try {
          ...
          (await this.archiver.saveMessage(ticket.id, message, true))
			...
			this.client.log.warn.tickets(
				'An error occurred whilst creating ticket',
				channel.id,
			);
		...
		if (currentHours[0] === currentHours[1] || now.isAfter(end)) {
			// staff have the day off or have finished for the day
			// first look for the next working day *this* week (after today)
			let nextIndex = workingHours.findIndex(
				(hours, i) => i > now.day() && hours[0] !== hours[1],
			);
			// if there isn't one, look for the next working day *next* week (before and including today's weekday)
			if (!nextIndex) {
				nextIndex = workingHours.findIndex(
					(hours, i) => i <= now.day() && hours[0] !== hours[1],
				);
			}
			if (nextIndex) {
				...
				await channel.send({
					embeds: [
						new ExtendedEmbedBuilder()
							...
							.setDescription(
								getMessage('ticket.working_hours.next.description', { timestamp }),
							),
					],
				});
			}
		} else if (now.isBefore(start)) {
			// staff haven't started working yet
			...
			await channel.send({
				embeds: [
					new ExtendedEmbedBuilder()
						...
						.setDescription(
							getMessage('ticket.working_hours.today.description', { timestamp }),
						),
				],
			});
		}
   ...
   */
	async claim(interaction) {
			...
			return await interaction.editReply({
				embeds: [
					new ExtendedEmbedBuilder({
						...
						.setDescription(
							getMessage('commands.slash.claim.not_staff.description'),
						),
				],
			});
		}
		...
		const openingMessage = await interaction.channel.messages.fetch(
			ticket.openingMessageId,
		);
		...
		await interaction.editReply({
			embeds: [
				new ExtendedEmbedBuilder()
					...
					.setDescription(
						getMessage('ticket.claimed', { user: interaction.user.toString() }),
					),
			],
		});
		...
		await interaction.editReply({
			embeds: [
				new ExtendedEmbedBuilder()
					.setColor(ticket.guild.primaryColour)
					.setDescription(
						getMessage('ticket.released', { user: interaction.user.toString() }),
					),
			],
		});
	...
	async beforeRequestClose(interaction) {
      ...
      !ticket.feedback
			...
			return await interaction.showModal(
				this.buildFeedbackModal(ticket.guild.locale, {
					next: 'requestClose',
   ...
   */
	async requestClose(interaction, reason) {
		...
		const getMessage = this.client.i18n.getLocale(ticket.guild.locale);
		const staff =
      interaction.user.id !== ticket.createdById &&
      (await isStaff(interaction.guild, interaction.user.id));
		...
		const embed = new ExtendedEmbedBuilder({
			...
			.setColor(ticket.guild.primaryColour)
			.setTitle(
				getMessage(`ticket.close.${staff ? 'staff' : 'user'}_request.title`, { requestedBy: interaction.member.displayName }),
			);

		if (staff) {
			embed.setDescription(
				getMessage('ticket.close.staff_request.description', { requestedBy: interaction.user.toString() }) +
          (ticket.guild.archive
          	? getMessage('ticket.close.staff_request.archived')
          	: ''),
			);
		}
		...
		this.$stale.set(ticket.id, {
			closeAt: ticket.guild.autoClose
				? Date.now() + ticket.guild.autoClose
				: null,
	...
	async finallyClose(ticketId, {
		...
		if (channel?.deletable) {
			...
			await channel.delete(
				'Ticket closed' + (member ? ` by ${member.displayName}` : '') + reason
					? `: ${reason}`
					: '',
			);
		}
			...
			if (creator) {
				...
				if (ticket.feedback) {
					embed.addFields({
						...
						value:
              Array(ticket.feedback.rating).fill('⭐').join(' ') +
